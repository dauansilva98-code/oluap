...
export default function ContasReceber() {
    ...
    const initAll = () => {
        ...
        options: {
          ...
          scales: {
            ...
            y: {
              ...
              ticks: { ...tickFn, callback: v => `R$ ${v / 1000}k` },
            },
          },
        },
      })
      ...
      donutChart.current = new window.Chart(donutRef.current, {
        ...
        data: {
          ...
          datasets: [{
            data: [28400, 17500, 9800, 5100, 2700],
            backgroundColor: ['#1D9E75', '#378ADD', '#7F77DD', '#BA7517', '#888780'],
            borderWidth: 0,
          }],
        },
        options: {
      ...
      agingChart.current = new window.Chart(agingRef.current, {
        ...
        data: {
          ...
          datasets: [{
            data: computeAging(INITIAL_COBRANÇAS),
            backgroundColor: ['#EF9F27', '#D85A30', '#A32D2D'],
            borderRadius: 4,
            borderWidth: 0,
          }],
        },
  ...
  // Update aging chart when cobranças changes
  useEffect(() => {
  ...
  const labelStyle  = (color) => ({ fontSize: 11, fontWeight: 500, color, marginBottom: 4 })
  const valueStyle  = (color) => ({ fontSize: 19, fontWeight: 500, color, lineHeight: 1.2 })
  const subStyle    = (color) => ({ fontSize: 11, color, marginTop: 2 })
    ...
    <>
        ...
        >
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-[420px]">
            <div className="flex items-center justify-between px-6 pt-5 pb-4 border-b border-slate-100">
              ...
              <button onClick={() => setModal(false)} className="text-slate-400 hover:text-slate-600 p-1 rounded-lg hover:bg-slate-50">
                <X size={16} />
              </button>
            </div>
            <div className="px-6 py-5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <label className="block">
                  ...
                  <select className={selectCls} value={form.cat} onChange={field('cat')}>
                ...
                <label className="block">
                  ...
                  <select className={selectCls} value={form.tipo} onChange={field('tipo')}>
              ...
              <div className="grid grid-cols-2 gap-3">
                <label className="block">
                  ...
                  <select className={selectCls} value={form.met} onChange={field('met')}>
                ...
                <label className="block">
                  ...
                  <select className={selectCls} value={form.recorrencia} onChange={field('recorrencia')}>
            ...
            <div className="flex justify-end gap-3 px-6 pb-5">
              <button onClick={() => setModal(false)} className="px-5 py-2.5 rounded-xl text-sm font-medium text-slate-500 border border-slate-200 hover:bg-slate-50 transition-colors">
                Cancelar
              </button>
              <button onClick={handleSalvar} className="px-5 py-2.5 rounded-xl text-sm font-semibold text-white bg-[#05121b] hover:bg-[#137789] transition-colors">
                Salvar
              </button>
            </div>
          </div>
      ...
      <div className="max-w-7xl mx-auto w-full" style={{ display: 'flex', flexDirection: 'column', gap: 28 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: 12 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <select
              ...
              className="border border-slate-200 rounded-xl px-3 py-2 text-sm text-[#05121b] bg-white outline-none focus:border-[#137789] transition-colors"
            >
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))', gap: 12 }}>
          ...
          {card('#EAF3DE', '#C0DD97',
            <>
              <p style={labelStyle('#3B6D11')}>Total a receber</p>
              <p style={valueStyle('#27500A')}>{fmtBRL(totalAReceber)}</p>
              <p style={subStyle('#3B6D11')}>{qtdAReceber} cobranças</p>
            </>
          )}
          {/* Card 2 — Inadimplentes */}
          {card('#FCEBEB', '#F7C1C1',
            <>
              <p style={labelStyle('#993C1D')}>Inadimplentes</p>
              <p style={valueStyle('#791F1F')}>{fmtBRL(inadimplente)}</p>
              <p style={subStyle('#993C1D')}>{atrasados.length} clientes</p>
            </>
          )}
          {/* Card 3 — Vencem em 7 dias */}
          {card('#FAEEDA', '#FAC775',
            <>
              <p style={labelStyle('#854F0B')}>Vencem em 7 dias</p>
              <p style={valueStyle('#633806')}>{fmtBRL(totalVencer7)}</p>
              <p style={subStyle('#854F0B')}>{vencendoLogo.length} cobranças</p>
            </>
          )}
          {/* Card 4 — Recebido no mês */}
          {card('#EAF3DE', '#C0DD97',
            <>
              <p style={labelStyle('#3B6D11')}>Recebido no mês</p>
              <p style={valueStyle('#27500A')}>{fmtBRL(recebido)}</p>
              <p style={subStyle('#3B6D11')}>↑ 12,4% vs abril</p>
            </>
          )}
          {/* Card 5 — Total emitido */}
          {card('#E6F1FB', '#B5D4F4',
            <>
              <p style={labelStyle('#185FA5')}>Total emitido</p>
              <p style={valueStyle('#0C447C')}>{fmtBRL(totalEmitido)}</p>
              <p style={{ fontSize: 11, color: '#185FA5', marginTop: 2 }}>mês atual</p>
            </>
          )}
          {/* Card 6 — Taxa inadimplência */}
          ...
          {atrasados.length > 0 && (
            <div style={{ background: '#FCEBEB', border: '0.5px solid #F09595', borderRadius: 12, padding: '12px 16px', display: 'flex', gap: 10, alignItems: 'flex-start' }}>
              ...
              <p style={{ fontSize: 12, color: '#791F1F', lineHeight: 1.6, margin: 0 }}>
                ...
                {atrasados.map((c, i) => (
                  <span key={c.id}>{c.desc.split(' — ')[0]} {fmtBRL(c.valor)} (venc. {c.venc}){i < atrasados.length - 1 ? ', ' : ''}</span>
                ))}.{' '}
          ...
          {vencendoLogo.length > 0 && (
            <div style={{ background: '#FAEEDA', border: '0.5px solid #EF9F27', borderRadius: 12, padding: '12px 16px', display: 'flex', gap: 10, alignItems: 'flex-start' }}>
              ...
              <p style={{ fontSize: 12, color: '#633806', lineHeight: 1.6, margin: 0 }}>
                ...
                {vencendoLogo.map((c, i) => (
                  <span key={c.id}>{c.desc.split(' — ')[0]} {fmtBRL(c.valor)} ({c.venc}){i < vencendoLogo.length - 1 ? ', ' : ''}</span>
                ))}.{' '}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2" style={{ gap: 14 }}>
          {/* Gráfico 1 — Linha dupla */}
          <div className="bg-white border border-slate-200" style={{ borderRadius: 12, padding: '1rem 1.2rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
              <p style={{ fontSize: 13, fontWeight: 500, color: '#05121b', margin: 0 }}>Recebimentos — últimos 6 meses</p>
            </div>
            {/* Legenda customizada */}
          ...
          {/* Componente 2 — Top clientes */}
          <div className="bg-white border border-slate-200" style={{ borderRadius: 12, padding: '1rem 1.2rem' }}>
            <p style={{ fontSize: 13, fontWeight: 500, color: '#05121b', marginBottom: 14, marginTop: 0 }}>Top clientes — maio</p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {TOP_CLIENTES.map(({ init, nome, cat, valor }) => (
                <div key={nome} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  {/* Avatar */}
                  <div style={{ width: 32, height: 32, borderRadius: '50%', background: '#E6F1FB', color: '#0C447C', fontSize: 11, fontWeight: 600, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    {init}
                  </div>
                  {/* Info + bar */}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2" style={{ gap: 14 }}>
          {/* Gráfico 3 — Rosca */}
          <div className="bg-white border border-slate-200" style={{ borderRadius: 12, padding: '1rem 1.2rem' }}>
            <p style={{ fontSize: 13, fontWeight: 500, color: '#05121b', marginBottom: 12, marginTop: 0 }}>Recebimentos por categoria</p>
          ...
          {/* Gráfico 4 — Aging */}
          <div className="bg-white border border-slate-200" style={{ borderRadius: 12, padding: '1rem 1.2rem' }}>
            <p style={{ fontSize: 13, fontWeight: 500, color: '#05121b', marginBottom: 12, marginTop: 0 }}>Aging — dias em atraso</p>
            <div style={{ position: 'relative', width: '100%', height: 200 }}>
              <canvas ref={agingRef} />
            </div>
            {/* Legenda */}
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px 16px', marginTop: 10 }}>
              {[
                { color: '#EF9F27', label: 'Até 15 dias'  },
                { color: '#D85A30', label: '16–30 dias'   },
                { color: '#A32D2D', label: '31–60 dias'   },
              ].map(({ color, label }) => (
                <div key={label} style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                  <div style={{ width: 8, height: 8, borderRadius: 2, background: color, flexShrink: 0 }} />
              ...
              {FILTROS.map(({ key, label }) => (
                <button
                  ...
                  style={{
                    ...
                    background: filtro === key ? '#05121b' : '#f8fafc',
                    color:      filtro === key ? '#fff'    : '#64748b',
                    borderColor: filtro === key ? '#05121b': '#e2e8f0',
                  }}
                >
                  {label}
                </button>
          ...
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', tableLayout: 'fixed', borderCollapse: 'collapse' }}>
              ...
              <thead>
                <tr style={{ borderBottom: '0.5px solid #e2e8f0', background: '#f8fafc' }}>
                  {['Cliente / descrição','Categoria','Vencimento','Método','Tipo','Status','Valor','Ação'].map(h => (
                    <th key={h} style={{ padding: '10px 12px', fontSize: 11, fontWeight: 500, color: '#94a3b8', textAlign: h === 'Valor' ? 'right' : 'left', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {h}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                  ...
                  return (
                    ...
                    >
                      ...
                      <td style={{ padding: '10px 12px' }}>
                        <span style={{ fontSize: 12, color: isAtrasado ? '#D85A30' : '#05121b', fontWeight: isAtrasado ? 500 : 400 }}>
                          {c.venc}
                        </span>
                      </td>
                      {/* Método */}
                      ...
                      {/* Tipo */}
                      <td style={{ padding: '10px 12px' }}>
                        <span style={{ fontSize: 11, fontWeight: 500, padding: '2px 8px', borderRadius: 999, background: tipoBadge.bg, color: tipoBadge.color, border: `0.5px solid ${tipoBadge.border}` }}>
                          {tipoBadge.label}
                        </span>
                      </td>
                      {/* Status */}
                      <td style={{ padding: '10px 12px' }}>
                        <span style={{ fontSize: 11, fontWeight: 500, padding: '2px 8px', borderRadius: 999, background: statusBadge.bg, color: statusBadge.color }}>
                          {statusBadge.label}
                        </span>
                      </td>
                      {/* Valor */}
                      <td style={{ padding: '10px 12px', textAlign: 'right' }}>
                        <span style={{ fontSize: 13, fontWeight: 500, color: valorColor }}>
                          {fmtBRL(c.valor)}
                        </span>
                      </td>
                      {/* Ação */}
                      <td style={{ padding: '10px 12px' }}>
                        {showReceber && (
                          <button
                            ...
                            style={{ padding: '3px 10px', borderRadius: 999, fontSize: 11, fontWeight: 500, background: '#E6F1FB', color: '#185FA5', border: '0.5px solid #B5D4F4', cursor: 'pointer', whiteSpace: 'nowrap', transition: 'opacity 0.15s' }}
                ...
                {filtrados.length === 0 && (
                  <tr>
                    <td colSpan={8} style={{ padding: '32px 12px', textAlign: 'center', fontSize: 13, color: '#94a3b8' }}>
